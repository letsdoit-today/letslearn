...
const AirWaterRefraction: React.FC = () => {
  ...
  const [n1, setN1] = useState(CONFIG.N1);
  const [n2, setN2] = useState(CONFIG.N2);
  ...
  const updateScene = useCallback((angle: number) => {
    ...
    rays.i.attr({ "stroke-width": 2 + 8 * (fr.R + fr.T)});
    ...
    if(fr.T > 0.01){
      ...
      rays.t.attr({ "stroke-width": 0, "stroke-opacity": 0 });
    }
  ...
  useEffect(() => {
    ...
    s.text("介质1 n₁").move(CONFIG.CENTER_X - 300, CONFIG.CENTER_Y - 40).fill("#7f8c8d").font({ size: "16px" });
    s.text("介质2 n₂").move(CONFIG.CENTER_X - 300, CONFIG.CENTER_Y + 20).fill("#3498dbff").font({ size: "16px" });
        ...
        <CardContent className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-80 space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <Label>介质1折射率 n₁: {n1.toFixed(2)}</Label>
                <Slider
                  value={[n1]}
                  ...
                  onValueChange={(vals) => {
                    ...
                    setIsPlaying(false);
                    setN1(vals[0]);
                    updateScene(angleDeg);
                  }}
                  />
              </div>
              <div className="space-y-1">
                <Label>介质2折射率 n₂: {n2.toFixed(2)}</Label>
                <Slider
                  value={[n2]}
                  min={1} max={2} step={0.1}
                  onValueChange={(vals) => {
